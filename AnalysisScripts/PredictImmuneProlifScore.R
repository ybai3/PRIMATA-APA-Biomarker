...
process_data<-function(cancertype,option){
  if(option=="immune"){
    ...
    df.ctrl1<-df.ctrl
    #df.ctrl1[,1:(ncol(df.ctrl1)-1)]<-log(df.ctrl1[,1:(ncol(df.ctrl1)-1)]+1)#####take log to FPM as the ctrl
    ## remove log transform, since the original unit was log2(RPM+1)
    ...
    # df1[,1:(ncol(df1)-1)]<-log(df1[,1:(ncol(df1)-1)]+1)
...
numCancer <- length(cancerType1)
result_Rsquare_case<-matrix(NA,num_simulation,numCancer)
result_Rsquare_ctrl<-matrix(NA,num_simulation,numCancer)
result_Rmse_case<-matrix(NA,num_simulation,numCancer)
result_Rmse_ctrl<-matrix(NA,num_simulation,numCancer)
result_corR_case<-matrix(NA,num_simulation,numCancer)
result_corR_ctrl<-matrix(NA,num_simulation,numCancer)
result_corP_case<-matrix(NA,num_simulation,numCancer)
result_corP_ctrl<-matrix(NA,num_simulation,numCancer)
result_feature_select_case <- vector(mode = "list", length = numCancer)
result_feature_select_ctrl <- vector(mode = "list", length = numCancer)
for(i1 in 1:numCancer){
  print(i1)
  ...
  result_feature_select_case[[i1]]<-result_case1_NoPrior$MIRNA
  ...
  result_feature_select_ctrl[[i1]]<-result_ctrl1_NoPrior$MIRNA
}
...
names(result_feature_select_case)<-cancerType1
names(result_feature_select_ctrl)<-cancerType1

Data<-list(result_Rsquare_case=result_Rsquare_case,result_Rmse_case=result_Rmse_case,result_corR_case=result_corR_case,result_corP_case=result_corP_case,result_feature_select_case = result_feature_select_case,
           result_Rsquare_ctrl=result_Rsquare_ctrl,result_Rmse_ctrl=result_Rmse_ctrl,result_corR_ctrl=result_corR_ctrl,result_corP_ctrl=result_corP_ctrl,result_feature_select_ctrl = result_feature_select_ctrl)
setwd("/ihome/hpark/yub20/SUR3UTR/GDC_TCGA_FPKM_analysis/logTransNumTS")
save(Data,file=paste0(Data_option,"_rep300_result_list.Rdata"))
