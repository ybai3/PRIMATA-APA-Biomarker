#' Divide data to train and test (outer loop)
#'
#' @description  \code{divide.data} is called in \code{trainModel} function to divide data.
#'
#' @param data  \code{trainModel} would give the input. Basically, the input is a data frame consisting predictor matrix and outcome score ( the last column). One of two data frames included in the list object returned by \code{process_data}.
...
#'
#' @details This is a function called in \code{trainModel} function. In each iterative run of model training, it splits whole dataset into train and test set, so that model trained in train set can be evaluated in the completely unseen test set.
#'
#'
#' @examples
#' div.data  <- divide.data(dataList$df, train.portion = 0.75)
#'
...
#' Predictive model training and testing in nested cross-validation.
#'
#' @description \code{trainModel} splits data into train set and test set. It trains model using elastic net regression and evaluate the performance by calculating R square and RMSE on test set.
#'
#' @param Data A data frame consisting predictor matrix and outcome score (last column). One of two data frames included in the list object returned by \code{process_data}.
#' @param num_simulation Desired number of repetitive runs.
...
trainModel <-function(Data,num_simulation,trainportion=0.75){
  ...
  for(i in 1:num_simulation) {
    div.data <- divide.data(Data, trainportion)
